...
#define MY_ALLOC_ZE_AND_CPY(to, size, from, alloc) \
  ...
  /* NOTE (AAE): the JBinding tree misspelled `to` as `p` here, which nulled
     SzReadHeader2's archive pointer whenever a zero-size block was copied
     (only reachable with a 0-file kName section) and segfaulted the reader.
     Upstream LZMA SDK uses `to`; this restores that. */
